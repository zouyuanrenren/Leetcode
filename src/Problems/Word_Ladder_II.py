...
1. build a graph that include as nodes all words in dict, start and end.
    two nodes are connected if they can be transformed to each other.
2. start from the start, perform the breadth-first search to find shortest paths to end.
'''
class Solution:
    ...
    def findLadders(self, start, end, dict):
        ...
        results = []
        queue = [[start]]   # candidate path from start to end
        visited = set([start])  # visited nodes in so far
        border = set()     # nodes visitable for new candidate path
        step = 0            # minimal distance to the end
        size = 1            # the current path size
        while len(queue):
            path = queue.pop(0)
            if step and len(path) > step:   # if the program start to expore pathes longer than the minimal, strop
                break
            if len(path) > size:            # when the path increases, update the visited and border
                size += 1
                visited |= border
                border = set()
            word = path[-1]
