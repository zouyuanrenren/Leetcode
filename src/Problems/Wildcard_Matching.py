...
class Solution:
    ...
    def isMatch(self, s, p):
        ...
        # consider the following, where | separats matching sub-strings:
        # S: ...|a...|b...|c...
        # P: ...|*a...|*b...|d...
        ...
        #    This indicates that we only need to remmeber the position of the last * and its matching pointer in S
        lastS = None
        lastM = None
        i1 = 0
        i2 = 0
        while i1 < len(s):
            if i2 < len(p) and (s[i1] == p[i2] or p[i2] == "?"): # find exact matching or ? matching
                i1+=1
                i2+=1
            elif i2 < len(p) and p[i2] == "*": # find *, we skip it in patten
                lastS = i2
                lastM = i1
                i2 += 1
